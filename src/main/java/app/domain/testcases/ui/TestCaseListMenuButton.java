...
public final class TestCaseListMenuButton extends Button {
    ...
    public TestCaseListMenuButton() {
        setUserData(14);
        getProperties().put(SVG_NAME_KEY, SVG_NAME);
        setText("");
        setFocusTraversable(false);
        getStyleClass().add("icon-btn");
    }

    public void install(StackPane leftRoot, Runnable onBeforeOpen) {
        if (leftRoot == null) return;

        this.onBeforeOpen = nz(onBeforeOpen);

        host = new RightAnchoredModal(leftRoot, this, this::buildMenuModal);
    ...
    public void setOnImportAction(Runnable r) {
        this.onImport = nz(r);
    }
    ...
    private VBox buildMenuModal() {
        ...
        buttonsBox.getChildren().addAll(
                buildActionButton("tc.menu.import", () -> onImport),
                buildActionButton("tc.menu.export", () -> onExport),
                buildActionButton("cy.menu.delete", () -> onDelete)
        );
    ...
    private Button buildActionButton(String key, Supplier<Runnable> actionSupplier) {
